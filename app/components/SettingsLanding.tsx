...
export default function SettingsLanding({ initialSettings, initialStats }: Props) {
  const [settings, setSettings] = useState<Settings>({
    fetch_interval_hours: initialSettings.fetch_interval_hours ?? "3",
    digest_size: initialSettings.digest_size ?? "5",
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-sm text-gray-300">
              ...
              <span className="text-gray-600 text-xs">(total, split evenly per source)</span>
            </div>
            <div className="flex items-center gap-2">
              <SaveIndicator state={feedLimitSave} />
              <input
                type="number"
                min={10}
                max={2000}
                value={settings.news_feed_limit}
                onChange={(e) => setSettings((s) => ({ ...s, news_feed_limit: e.target.value }))}
                onBlur={() =>
                  saveSetting("news_feed_limit", settings.news_feed_limit, setFeedLimitSaving, setFeedLimitSaved, setFeedLimitError)
                }
          ...
          <p className="text-xs text-gray-600 pl-6">Divided evenly across active sources. With 6 sources and limit 100, each gets ~16 items. Higher values increase load time.</p>
        </div>
